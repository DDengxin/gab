...
public class GUserInfoServiceImpl implements GUserInfoService {
    ...
    public Result getSmsCode(GUserInfo gUserInfo) {
        ...
        boolean notExistsUserFlag = gUserInfoDao
                .findAll(Specifications.where((c) -> c.eq("userMtel", gUserInfo.getUserMtel()))).size()<=0;
        if (notExistsUserFlag) {
            SysSmsLog sysSmsLog = sysSmsService.send(SysSmsLog.GenSms(gUserInfo.getUserId(), gUserInfo.getUserMtel(), SessionUser.getDefaultCorp(), SmsType.GABREGISTER, RandomUtil.randomNumbers(6)));
    ...
    @Override
    public Result updatePwd(UsersPwdVo usersPwdVo) {
        GUserInfo user = gUserInfoDao.findByUserId(usersPwdVo.getUserName());
        //原密码不相等
//        if (!(user.getUserPwd().equals(usersPwdVo.getOldPwd()))) {
//            return Result.error("原密码输入有误，请重新输入");
//        } else {
//            user.setUserPwd(usersPwdVo.getNewPwd());
//            gUserInfoDao.update(user);
...
//        }
    ...
    *//**
    ...
    @Override
    public Result getGUserInfoStatus(String supplyNote, String status) {
        String  flag =	gUserInfoDao.getGUserInfoStatus(supplyNote);
        if (status.equals(flag)) {
            return Result.resultOk("操作成功！");
        }
        return Result.error("该单不是“" + status + "”状态,或该数据非平台用户添加,不能操作！");
    ...
    @Override
    public Map<String, Object> getIsSubmitApprove() {
        SessionUser sessionUser = SessionUser.SessionUser();
        GUserInfo model = sessionUser.getgUserInfo();
        /**
         * 环节类型（1开始环节，2普通环节，3结束环节）
         * 来源表sys_workflow_like 表
         */
        String  sql = "select \n" +
                "(case when sys_workflow_applyfor.id is null then'未启动'   \n" +
                "when  sys_workflow_applyfor.process_type  =  '1'   then  '驳回' " +
                " when sys_workflow_applyfor.process_type  ='2' then '审批'" +
                "else '结束' end)  status   \n" +
                "from g_userinfo  left join  sys_workflow_applyfor " +
                "on g_userinfo.user_id = sys_workflow_applyfor.business_id  where g_userinfo.user_id = '"+model.getUserId()+"'";
        return gUserInfoDao.QueryToFirstMap(sql);
    }
}
