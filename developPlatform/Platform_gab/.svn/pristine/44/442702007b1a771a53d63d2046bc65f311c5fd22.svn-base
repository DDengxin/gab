...
import com.tengzhi.business.platform.shopping.dao.*;
...
public class ShoppingServiceImpl implements ShoppingService {
    ...
    public Result addShoppingCart(String dataMan, String productNum) throws Exception {
        ...
        if (sessionUser.getgUserInfo() == null) {
            return Result.error("该账户非哥爱帮平台账户");
        }
        List<ShoppingCart> cart = shoppingCartDao.getproduct(productNum, sessionUser.getgUserInfo().getUserId());

        if (dataMan.equals(sessionUser.getgUserInfo().getUserId())) {
        ...
        //如果当前用户的购物车中没有存在的商品编号
            ...
            shoppingCart.setProductCompany(dataMan);
            shoppingCart.setOperator(sessionUser.getgUserInfo().getUserId());
            shoppingCart.setOperatorCompany(sessionUser.getCorpId());
            shoppingCart.setOwner(sessionUser.getgUserInfo().getUserId());
...
//        String userId = SessionUser.SessionUser().getUserId();
        String userId = SessionUser.SessionUser().getgUserInfo().getUserId();
	...
	private Result setG_EXsContract(Map<String, Object> map) {
        ...
        xsContract.setDataMan(user.getgUserInfo().getUserId());
        ...
        xsContract.setHtCustomer(user.getgUserInfo().getUserId());
        //创建合同详情
        ...
        for (int i = 0; i < cpcodes.size(); i++) {
            ...
            g_EXsContractDetailed.setDataMan(user.getgUserInfo().getUserId());
