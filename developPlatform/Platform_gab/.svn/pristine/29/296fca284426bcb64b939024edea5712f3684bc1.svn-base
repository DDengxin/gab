...
public class ProductServiceImpl implements ProductService {
	...
	public Result saveNeed(Product product) throws Exception {
		...
		product.setDataMan(user.getgUserInfo().getUserId());
		product.setProductFlag("登记");
		product.setSupplyId(productDao.getSupply(user.getgUserInfo().getUserId()));
...
//				" e_js_cpcode a left join sys_user b on a.data_man=b.user_id left join  g_supply c on c.supply_note=b.supply_id ";
		...
		String sqlPlus = "SELECT\n" +
				"\tcpcode_fl,\n" +
				"\tf_gab_getname ( 'GETGCOMPANYNAMEBYERPUSER', data_man, NULL, data_corp ) supply_name,\n" +
				"\tcpcode_price,\n" +
				"\tcpcode_name,\n" +
				"\tf_get_param_name ( '检验档案', cpcode_check, 'cn' ) cpcode_check,\n" +
				"\tcpcode_id,\n" +
				"\tcpcode_name_en,\n" +
				"\tcpcode_size,\n" +
				"\tf_get_param_name ( '计量单位', cpcode_bz, 'cn' ) cpcode_bz,\n" +
				"\tcpcode_xp,\n" +
				"\t( SELECT param_name FROM sys_param WHERE param_mod = '平台参数' AND param_type = '表单配置' AND param_id = cpcode_xp ) cpcode_xp_name,\n" +
				"\tf_get_param_name ( '大包单位', cpcode_dunit, 'cn' ) cpcode_dunit,\n" +
				"\tcpcode01,\n" +
				"\tcpcode02,\n" +
				"\tcpcode03,\n" +
				"\tcpcode_flag,\n" +
				...
				"\tproduct_description,\n" +
				"\tproduct_sales,\n" +
				"\t( SELECT org_name FROM sys_org WHERE org_id = data_corp ) data_corp_name,\n" +
				"\tdata_corp,\n" +
				"\tdata_rq,\n" +
				"\tservice_description,\n" +
				"\tcover_picture,\n" +
