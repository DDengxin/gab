$(function () {
	...
	//行业分类值改变事件
	$("#cpcodeXp").change(function () {
		formConfig({classify:$("#cpcodeXp").val(),index:2,show:'front'});
	});
...
function getNeedService() {
	...
	if (type=="产品需求") {
		getSysParams("cpcodeXp", "技术", "夹具大类", "CP");
	}else {
		getSysParams("cpcodeXp", "技术", "服务大类", "HW");
	}
}
	...
	if (type=="产品需求"){
		...
		$.ajax({
			...
			success: function (res) {
				$("span[name='introduceName']").html("需求介绍<i>*</i>");
				$("span[name='applyName']").html("应用需求<i>*</i>");
				for(var i=0;i<res.length;i++){
					var _name=res[i].paramValue1;
					var _value=res[i].paramName;
					$("span[name='"+_name+"_Name']").html(_value+"<i>*</i>")
		...
		$.ajax({
			...
			success: function (res) {
				$("span[name='introduceName']").html("服务介绍<i>*</i>");
				$("span[name='applyName']").html("应用服务<i>*</i>");
				for(var i=0;i<res.length;i++){
					var _name=res[i].paramValue1;
					var _value=res[i].paramName;
					$("span[name='"+_name+"_Name']").html(_value+"<i>*</i>")
