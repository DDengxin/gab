...
public class NeedServiceImpl implements NeedService {
	...
	private String needSql() {
		return "select need_note \"needNote\",(CASE WHEN need_form = 'CP' THEN f_getparamname ( 'GETBYPARENTID', cpcode_xp, '夹具大类', '技术', 'CP', data_corp ) WHEN need_form = 'HW' THEN f_getparamname ( 'GETBYPARENTID', cpcode_xp, '服务大类', '技术', 'HW', data_corp ) end) \"typeName\",need_province \"needProvince\",need_city \"needCity\",to_char(end_time,'yyyy-MM-dd') \"endTime\",need_img \"needImg\",('/system/upload/getImage?line_primary='||need_img) \"imgSrc\",need_file \"needFile\",need_flag \"needFlag\",data_man \"dataMan\",to_char(data_date,'yyyy-MM-dd') \"dataDate\",data_corp \"dataCorp\",f_getname('GETCORPNAME', '', '', data_corp) \"corpName\",duty_cycle \"dutyCycle\",apply_man \"applyMan\" ,cpcode_name \"cpcodeName\",cpcode_size \"cpcodeSize\",cpcode_size_en \"cpcodeSizeEn\",introduce \"introduce\",cpcode_fl \"cpcodeFl\",cpcode_xp \"cpcodeXp\",case when need_form='CP' then '产品' when need_form='HW' then '服务' else '' end as \"formType\" from g_need ";
	}
	@Override
	...
	public Result saveNeed(needVo needVo) throws Exception{
		...
		g_Need.setDataMan(sessionUser.getgUserInfo().getUserId());
		g_Need.setDataDate(new Date());
		g_Need.setApplyCorp(sessionUser.getCorpId());
		g_Need.setApplyMan(sessionUser.getgUserInfo().getUserId());
	...
	public List<Map<String, Object>> getFirmNeed(String firm) throws IOException {
		String sql = "select supply_note \"supplyNote\",supply_name \"supplyName\",'/system/upload/getImage?line_primary='||head_img \"headImg\",supply_info \"supplyInfo\" from g_supply where supply_type = '" + firm + "' and status = '核准'  order by register_date desc limit 4";
