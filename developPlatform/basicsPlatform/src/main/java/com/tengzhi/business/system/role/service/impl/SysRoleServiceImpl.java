...
public class SysRoleServiceImpl implements SysRoleService {
    ...
    public Result findAll(BaseDto baseDto) {
        ...
        String where = SqlJoint.where(c -> {
            c.eq("data_Corp",  map.get("dataCorp"));
            c.contains("role_Id", map.get("roleId"));
            c.contains("role_Name", map.get("roleName"));
            c.contains("is_Forbidden", map.get("isForbidden"));
        }, true);
        String sql = " select  role_id, parent_id, role_name, gen_time, modify_time, description, is_forbidden, role_ord, data_chart, data_chart_two, data_corp,f_getname('GETCORPEXP','','',data_corp) \"corpName\",f_getname('CHARTNAME',data_chart,'',data_corp) \"chartName\",f_getname('CHARTNAME',data_chart_two,'',data_corp) \"chartTwoName\" from sys_role " + where;
        return sysRoleDao.QueryMapPageList(baseDto, sql, true).getResult();
    }
