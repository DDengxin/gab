    ...
    function getValueArrayFromClipboard(text) {
        ...
        function splitLines(text) {
            ...
            for (var i = 0, l = text.length; i < l; i++) {
                ...
                //修复多行复制问题20190920
                if (i === l - 1) {
                    array.push(text.substring(start, l))
                }
            }
...
//            selectCellRange(me.currentCell, me.currentCell);
    ...
    destroy: function () {
        ...
        $(grid.el).unbind('keydown');
        $(grid.el).unbind("mousedown");
        $(document).unbind("mousemove.excel-selection", handleMove);
        $(document).unbind("mouseup.excel-selection", handleEnd);
        grid.un('currentcellchanged');
    }
