package com.tengzhi.business.sale.processProduct.processContract.service.impl;
...
public class ProcessContractServiceImpl implements ProcessContractService {
	...
	public BasePage<Map<String, Object>> getSrchBottomList(BaseDto baseDto) throws IOException {
		...
		String sqlString=" select a.ht_no,a.ht_mo,a.ht_id,a.ht_code,a.ht_sl,a.ht_price,a.ht_je,to_char(a.ht_jq,'yyyy-MM-dd') ht_jq,a.ht_sm,a.ht_flag,a.data_man,a.data_rq,a.data_corp,a.ht_requirements, "
				+ "(select string_agg( param_name,',') from sys_param c where strpos(a.ht_standard,c.param_id)>0 and  c.param_mod='技术'  )ht_standard_name,"
				...
				+ "f_getparamname('GETCPCODESIZE',b.cpcode_size,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_name, "
				+ "f_getparamname('GETCPCODESIZEEN',b.cpcode_size_en,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_en_name,"
				+ "f_getparamname('GETCPCODEXP',b.cpcode_xp,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"')  cpcode_xp_name,"
 ...
 */
	...
	public Result confirm(String htNote) {
		eXsContractDao.updateFlag(htNote, "确认");
	...
	@Override
	public BasePage<EXsContractDetailedWl> getSrchBottomWlList(BaseDto baseDto) throws IOException {
		Map<String, String> map = baseDto.getParamsMap();
		String where ="and a.ht_no= '"+map.get("htNo")+"'";
		String sqlString=" select a.* "
				+ ",f_getparamname('GETBCPCODENAME',b.cpcode_name,'','技术','YL','"+   SessionUser.getCurrentCorpId()   +"')  cpcode_name,"
				+ "  b.cpcode_size ,"
				+ "f_getparamname('GETBYCPCODEFL',b.cpcode_fl,'','技术','YL','"+   SessionUser.getCurrentCorpId()   +"')cpcode_fl,"
				+ "b.cpcode_xp,f_getparamname('GETBYPARENTID',b.cpcode_bz,'计量单位','技术','YL','"+   SessionUser.getCurrentCorpId()   +"') cpcode_bz ,"
				+ "f_getparamname('GETCPCODESIZE',b.cpcode_size,'','技术','YL','"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_name,"
				+ " f_getparamname('GETCPCODESIZEEN',cpcode_size_en,'','技术','YL','"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_en_name,\r\n" + 
				""
				+ "f_getparamname('GETCPCODEXP',b.cpcode_xp,'','技术','YL','"+   SessionUser.getCurrentCorpId()   +"')  cpcode_xp_name"
				+ " from e_xs_contract_detailed_wl a,e_js_cpcode b  "
				+ "  where  a.wl_code=b.cpcode_id "+where ;
		return eXsContractDetailedWlDao.QueryPageLists(baseDto,sqlString);
	}

	@Override
	public EXsContract wlsave(ProcessContractWlVo eXsContractvo) throws Exception {
	
		if(!eXsContractvo.getAddedList().isEmpty()) {
			eXsContractvo.getAddedList().forEach( item -> {
				eXsContractDetailedWlDao.save(item);
			});
		}
		if (!eXsContractvo.getDeletedList().isEmpty()) {
			eXsContractDetailedWlDao.deleteAll(eXsContractvo.getDeletedList());
		}
		if(!eXsContractvo.getModifyedList().isEmpty()){
			eXsContractvo.getModifyedList().forEach( item ->{
				eXsContractDetailedWlDao.dynamicSave(item,eXsContractDetailedWlDao.findById(item.getId()).orElse(null));
			});
		}
		return  eXsContractvo.geteXsContract();
	}
