...
public class ECgContractSqlDaoImpl  extends BasicsDaoImpl<ECgContract, String> implements ECgContractSqlDao{
	...
	public List findList(BaseDto dto) {
		...
		String  sql="  select  a.*,f_get_param_name('采购合同',ht_type,'"+   SessionUser.getCurrentCorpId()   +"','cn') httypename,f_getname('GETDWEXP',ht_supplier,'',a.data_corp) htsupplierexp,f_get_param_name('交易币种',ht_bz,'"+   SessionUser.getCurrentCorpId()   +"') bzname "
				+ ",f_getparamname('GETBCPCODENAME',cpcode_name,'','技术',cg_stype,'"+   SessionUser.getCurrentCorpId()   +"')  cpcodename,f_getparamname('GETBYCPCODEFL',cpcode_fl,'','技术',cg_stype,'"+   SessionUser.getCurrentCorpId()   +"') cpcodefl,COALESCE(b.in_js,0) injs,COALESCE(b.in_sl,0) insl,COALESCE(b.in_zl,0) inzl "
