...
public class WarehouseAllotServiceImpl implements WarehouseAllotService {
	...
	public BasePage<ECkOut> getSrchTopList(BaseDto baseDto) throws Exception {
		...
		String sqlWhere = " where   out_type = '"+map.get("outType")+"' and  out_act in (select param_id from sys_param where param_name ='"+ WarehouseActionCaliber.dbck+"' )   ";
	...
	public Map<String, Object> getKcDetailedList(BaseDto baseDto) throws Exception {
			...
			rmap.put("data", warehouseAllotDao.QueryListMap(sql));
	...
	public Result save(ECkAllotVo vo) throws Exception {
		...
		String parameterId = eCkOutDao.getSingleResult("select f_get_in_out_act('GETACT',?1,?2,?3) "
				, WarehouseActionCaliber.dbck,vo.geteCkAllot().getOutType(),securityUser.getCorpId());
		SysParams sysParams = sysParamService.findByParameterId(parameterId, "仓库收发");
		String note = sysGenNoteService.getInOutNote(ECkOut.class,vo.geteCkAllot().getOutType(), WarehouseActionCaliber.dbck,securityUser.getCorpId());
	...
	public Result update(ECkAllotVo vo) throws Exception {
		...
		SessionUser sessionUser = SessionUser.SessionUser();
		String parameterId = eCkOutDao.getSingleResult("select f_get_in_out_act('GETACT',?1,?2,?3) "
				, WarehouseActionCaliber.dbck,vo.geteCkAllot().getOutType(),sessionUser.getCorpId());
		SysParams sysParams = sysParamService.findByParameterId(parameterId, "仓库收发");
	...
	public Result confirm(String outNote,String outType) throws Exception {
		SessionUser securityUser = SessionUser.SessionUser();
		String parameterId = eCkOutDao.getSingleResult("select f_get_in_out_act('GETACT',?1,?2,?3) "
				, WarehouseActionCaliber.dbrk,outType,securityUser.getCorpId());
		String outAllotNote = sysGenNoteService.getInOutNote(ECkOut.class,outType, WarehouseActionCaliber.dbrk,securityUser.getCorpId());
		warehouseAllotDao.confirm(outAllotNote,securityUser.getUserId(),outNote);
		warehouseAllotDao.insertAllot(outAllotNote, parameterId, securityUser.getUserId(), outNote);
