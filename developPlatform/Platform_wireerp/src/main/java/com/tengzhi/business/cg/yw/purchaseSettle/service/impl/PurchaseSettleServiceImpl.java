...
public class PurchaseSettleServiceImpl implements PurchaseSettleService {
    ...
    public BasePage<ECkIn> getSrchTopList(BaseDto baseDto) throws Exception {
        ...
        String sqlWhere = " where  data_corp='"+securityUser.getCorpId()+"' and in_flag in ('结算','确认') and in_act in (select param_id from sys_param where param_name in ('"+ WarehouseActionCaliber.cgrk+"','"+ WarehouseActionCaliber.cgth+"') ) ";
	...
	public BasePage<Map<String, Object>> getBottomList(BaseDto baseDto) throws IOException {
		Map<String, String> map = baseDto.getParamsMap();
		Object inNote = map.get("inNote");
		String sql ="select f_get_param_name('仓库收发',in_act,'"+   SessionUser.getCurrentCorpId()   +"') cwactname,f_get_param_name('库房档案',in_lib,'"+   SessionUser.getCurrentCorpId()   +"') cwlibname,f_get_param_name('交易币种',in_bz,'"+   SessionUser.getCurrentCorpId()   +"') bzname,f_get_param_name('产品大类',in_type,'"+   SessionUser.getCurrentCorpId()   +"') cwtypename,f_getparamname('GETBYPARENTID',b.cpcode_bz,'计量单位','技术','','"+   SessionUser.getCurrentCorpId()   +"')  cpcode_bz ,f_getname('GETDWEXP',in_customer,'',a.data_corp) cwcustomername, "
				+ " to_char(now(),'yyyy-MM-dd') cw_fkrq,to_char(in_rq,'yyyy-MM-dd') cw_rq,in_customer cw_dw,in_note cw_note,in_lib cw_lib,"
				...
				+ " f_getparamname('GETBYCPCODEFL',b.cpcode_fl,'','技术',b.cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"') as cpcode_fl,cpcode_xp,"
