package com.tengzhi.business.sale.domesticTrade.domesticTradeContract.service.impl;
...
public class DomesticTradeServiceImpl implements DomesticTradeService {
    ...
    public EXsContract save(EXsContractVo eXsContractVo) throws Exception {
        ...
        eXsContractVo.geteXsContract().setDataMan(securityUser.getUserId());
        ...
        if (!eXsContractVo.getAddedList().isEmpty()) {
            for (EXsContractDetailed item:eXsContractVo.getAddedList()){
                ...
                item.getHtItemData().forEach(model->{
                    ...
                    model.setHtMo(item.getHtMo());
                    model.setDataDate(new Date());
                    model.setDataCorp(securityUser.getCorpId());
                    model.setItemSid(UUID.randomUUID().toString());
                    eXsContractDetailedItemDao.save(model);
                });
                //eXsContractDetailedItemDao.save(item.getHtItemData());
            };
        }
        if (!eXsContractVo.getDeletedList().isEmpty()) {
            eXsContractDetailedDao.deleteAll(eXsContractVo.getDeletedList());
            eXsContractVo.getDeletedList().forEach(item->{
                eXsContractDetailedItemDao.deleteByHtMo(item.getHtMo());
            });
        }
    ...
    public void update(EXsContractVo eXsContractVo) {
        ...
        if (!eXsContractVo.getAddedList().isEmpty()) {
            for (EXsContractDetailed item:eXsContractVo.getAddedList()){
                ...
                item.setHtId(sid);
                sid++;
                item.setHtMo(item.getHtNo() + "-" + item.getHtId());
                item.setHtFlag("登记");
                eXsContractDetailedDao.save(item);
                item.getHtItemData().forEach(model->{
                    model.setDataMan(securityUser.getUserId());
    ...
    @Override
    public List<Map<String,Object>> getItemList(String htStype) {

        String sqlString=" select param_id ht_item_code,param_name ht_item_code_name,'' ht_item_value from sys_param where param_value1='"+htStype+"'  order by param_ord " ;
        return eXsContractDetailedDao.QueryToMap(sqlString);
    }

    @Override
    public List<Map<String, Object>> getDetailedList(BaseDto baseDto) throws IOException {
        Map<String, String> map = baseDto.getParamsMap();
        String where = "and a.ht_no= '" + map.get("htNo") + "'";
        String sqlString = " select a.ht_no,a.ht_mo,a.ht_id,a.ht_code,a.ht_sl,a.ht_price,a.ht_je,to_char(a.ht_jq,'yyyy-MM-dd') ht_jq,a.ht_sm,a.ht_flag,a.data_man,"
                + "a.data_corp,a.ht_requirements,(select string_agg( param_name,',') from sys_param c where strpos(a.ht_standard,c.param_id)>0 and  c.param_mod='技术'  )ht_standard_name,"
                + "ht_standard ,ht_customer_associated_remarks,ht_customer_associated, " +
                " b.cpcode_name,f_getparamname('GETBCPCODENAME',b.cpcode_name,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"')  cpcode_name_name,  "
                + " b.cpcode_size ,b.cpcode_size_en,to_char(coalesce(ht_yjq,ht_jq),'yyyy-mm-dd') ht_yjq ,"
                + " f_getparamname('GETCPCODESIZE',cpcode_size,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_name,"
                + " f_getparamname('GETCPCODESIZEEN',cpcode_size_en,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_en_name,"
                + " cpcode_xp,f_getparamname('GETCPCODEXP',cpcode_xp,'','技术',ht_stype,'"+   SessionUser.getCurrentCorpId()   +"')  cpcode_xp_name,"
                + " b.cpcode_fl,f_getparamname('GETBYCPCODEFL',b.cpcode_fl,'','技术',c.ht_stype,'"+   SessionUser.getCurrentCorpId()   +"')cpcode_fl_name,"
        ...
        return eXsContractDetailedDao.QueryToMap(sqlString).stream().map(row -> {
            return new HashMap<String, Object>() {{
                putAll(row);
                put("htItemData", eXsContractDetailedItemDao.QueryToMap("select ht_mo,data_corp,ht_item_code,ht_item_value,COALESCE(ht_item_value_reply,ht_item_value) ht_item_value_reply,f_get_param_name('线材项目',ht_item_code,data_corp,'')ht_item_code_name,item_sid from e_xs_contract_detailed_item items where ht_mo = '"+row.get("htMo")+"'"));
            }};
        }).collect(Collectors.toList());
    }

    @Override
    public Result updateTerms(EXsContract eXsContract) {
        eXsContractDao.dynamicSave(eXsContract, eXsContractDao.findById(eXsContract.getHtNo()).orElse(null));
