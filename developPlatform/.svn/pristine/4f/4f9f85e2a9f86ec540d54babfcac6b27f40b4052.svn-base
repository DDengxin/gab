...
public class QualityCertificateServiceImpl implements QualityCertificateService {
	...
	public BasePage<QualityCertificate> getSrchTopList(BaseDto baseDto) throws IOException {
		...
		if(ObjectUtil.isNotEmpty(map.get("srchNote"))) {
            where+=" and a.zm_note like '%"+map.get("srchNote")+"%'";
        }
		if(ObjectUtil.isNotEmpty(map.get("srchDeliveryNo"))) {
			where += " and a.zm_delivery_no like '%" + map.get("srchDeliveryNo") + "%'";
		}
		if(ObjectUtil.isNotEmpty(map.get("srchLotNo"))) {
			where += " and a.zm_lot_no like '%" + map.get("srchLotNo") + "%'";
		}
		if(ObjectUtil.isNotEmpty(map.get("srchFlag"))) {
			where+=" and a.zm_flag='"+map.get("srchFlag")+"' ";
		}
	...
	public Result getSrchBottomList0000(BaseDto baseDto) throws IOException {
		...
		if(ObjectUtil.isNotEmpty(map.get("srchDeliveryNo"))) {
			sqlWhere += " and zm_delivery_no like '%" + map.get("srchDeliveryNo") + "%'";
		}
		if(ObjectUtil.isNotEmpty(map.get("srchLotNo"))) {
			sqlWhere += " and zm_lot_no like '%" + map.get("srchLotNo") + "%'";
		}
	...
	public Result saveData(Map<String,Object> map) throws Exception {
		...
		body.forEach(item->{
			...
			cate.setDataMan(securityUser.getUserId());
	...
	public BasePage<ECkOut> getOutList(BaseDto baseDto) throws IOException {
		Map<String, String> map = baseDto.getParamsMap();
		String sqlWhere = " where a.out_note not in(select zm_delivery_no from e_pz_quality_certificate) and a.out_code=b.cpcode_id and out_act in (select param_id from sys_param where param_name ='"+ WarehouseActionCaliber.xsck+"' )   ";
		if(ObjectUtil.isNotEmpty(map.get("srchFlag"))) {
	...
	@Override
	public  Map<String, Object> getzmNoteAlone(String zmNote) {
		//return qualityCertificateDao.getzmNoteAlone(zmNote);
		
		//SELECT  zmRq,zmNote,zmDeliveryNo FROM QualityCertificate  where zmNote = :zmNote GROUP BY  zmRq,zmNote,zmItem,zmCustomer,zmDeliveryNo,dataMan,zmFla
		return qualityCertificateDao.QueryToFirstMap("SELECT  zm_rq,zm_note,zm_delivery_no FROM e_pz_quality_certificate  where zm_note = '"+zmNote+"' GROUP BY  zm_rq,zm_note,zm_item,zm_customer,zm_delivery_no,data_man,zm_flag");
	}
