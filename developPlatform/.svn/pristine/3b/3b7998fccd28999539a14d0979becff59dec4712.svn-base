...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
     ...
     * @param CompanyData
     */
    window.listTplGroup = function (d, dataList, type, innerFlag) {

        var TYPE = 'friend';
        // TODO:
        var spread = false;
        if ('groups' == type) {
            var layimTabContentClass = null == innerFlag ? 'layim-tab-content' : '';
            var layShowClass = innerFlag == null ? 'layui-show' : '';
            var ulStart = '<ul class="layui-unselect ' + layimTabContentClass + ' ' + layShowClass + ' layim-list-friend">';
            var ulEnd = '</ul>';
            window.firstUl = '';
            var groupLi = '';
            if ((dataList || []).length <= 0) {
                ...
                for (var i = 0, max = dataList.length; i < max; i += 1) {
                    var row = dataList[i];

                    // 计算群组人数
                    function countUser(dept) {
                        var count = 0;
                        $(dept).each(function (index, dept) {
                            if (null != dept.groups && dept.groups.length > 0) {
                                count += countUser(dept.groups)
                            }
                            if (null != dept.list && dept.list.length > 0) {
                                count += dept.list.length;
                            }
                        });
                        return count;
                    }

                    var count = countUser(row);
                    if (count == 0) {
                        groupLi += '';
                    } else {
                        groupLi += ['<li>'
                            , '<h5 layim-event="spread" lay-type="' + spread + '">'
                            , '<i class="layui-icon">' + ('true' == String(spread) ? '&#xe61a;' : '&#xe602;') + '</i>'
                            , '<span>' + (row.groupname || "未命名分组" + i) + '</span>'
                            , '<em>(<cite class="layim-count"> ' + count + '</cite>)</em></h5>'
                            , ((row.groups || []).length > 0 ? listTplGroup(d, row.groups, 'groups', true) : '')
                            , listTplGroup(d, row.list, 'list', true)
                            , '</li>'
                        ].join('');
                    }
            ...
            return ulStart + groupLi + ulEnd;
        } else if ('list' === type) {
            var ulStart = '<ul class="layui-layim-list  ' + ('true' == String(spread) ? 'layui-show' : '') + ' " >';
            var ulEnd = '</ul>';
            var userLi = '';
            if ((dataList || []).length <= 0) {
                userLi += '';
                ...
                for (var i = 0, max = dataList.length; i < max; i += 1) {
                    var row = dataList[i];
                    userLi += ['<li layim-event="chat" data-type="' + TYPE + '"  data-index="' + row.id + '" class="layim-' + (TYPE + row.id + (row.status === 'offline' ? ' layim-list-gray ' : '')) + '"   >'
                        , '<img src="' + row.avatar + '" />'
                        , '<span> ' + (row.username || row.groupname || row.name || "佚名") + '</span> '
                        , '<p>' + (row.remark || row.sign || "") + '</p>'
                        , '<span class="layim-msg-status">new</span>'
                        , '</li>'
                    ].join('');
                }
            }
            return ulStart + userLi + ulEnd;
        }
    };
    ...
    var listTpl = function (options) {
        ...
        return ['{{# var length = 0; layui.each(' + options.item + ', function(i, data){ length++; }}'
            , '<li layim-event="chat" onmousedown="flockTrigger(\'{{data.id}}\')" data-type="' + options.type + '" data-index="{{ ' + (options.index || 'i') + ' }}" class="layim-'
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        , ' layim-this'
        , '{{# } }}'
        , '" title="组织架构" layim-event="tab" lay-type="friend">&#xe641;</li>'
        , '<li class="layui-icon'
        , '{{# if(!d.base.isgroup){ }}'
        , ' layim-hide'
        ...
        // , '{{# if(!d.base.iscompany){ }}'
        // , ' layim-hide'
        // , '{{# } else if(!d.base.iscompany) { }}'
        // , ' layim-this'
        // , '{{# } }}'
        // , '" title="组织架构" layim-event="tab" lay-type="company">&#xe614;</li>'
        // END 自定义
        ...
        // if(d.base.isfriend){ }}layui-show{{# } }} layim-list-friend">'
        // , '{{# layui.each(d.friend, function(index, item){ var spread =
        // d.local["spread"+index]; }}'
        // , '<li>'
        // , '<h5 layim-event="spread" lay-type="{{ spread }}"><i
        // class="layui-icon">{{# if(spread === "true"){ }}&#xe61a;{{# } else {
        // }}&#xe602;{{# } }}</i>
        // <span>{{ item.groupname||"未命名分组"+index }}</span><em>(<cite
        // class="layim-count"> {{ (item.list||[]).length }}</cite>)</em></h5>'
        ...
        // , '<li><ul class="layui-layim-list layui-show"><li
        // class="layim-null">暂无联系人</li></ul>'
        // , '{{# } }}'
        // , '</ul>'

        // 自定义
        , '{{ listTplGroup(d,d.friend,"groups") }}'
        // END 自定义
        ...
        , '{{# if(d.base.find){ }}'
        , '<li class="layui-icon layim-tool-find" layim-event="find" title="查找群聊">&#xe61f;</li>'
        , '{{# } }}'
        , '{{# if(d.base.groupChat){ }}'
        , '<li class="layui-icon layim-tool-find" layim-event="groupChat" title="创建群聊">&#xe770;</li>'
        , '{{# } }}'
        , '{{# if(d.base.msgbox){ }}'
        , '<li class="layui-icon layim-tool-msgbox" layim-event="msgbox" title="系统消息">&#xe645;<span class="layui-anim"></span></li>'
        , '{{# } }}'
        , '<li class="layui-icon layim-tool-skin" layim-event="skin" title="设置背景">&#xe631;</li>'
        , '{{# if(!d.base.copyright){ }}'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
        ...
        , '{{# }; }}'
        , '</div>'
        , '<div class="layim-chat-textarea" contenteditable="true"></div>'
    ...
    var cache = {message: {}, chat: []}, init = function (options) {
        ...
        mine = init.mine || {}
            ...
            var local = layui.data('layim')[mine.id] || {}, obj = {
                ...
                , local: local // 本地数据
                , mine: mine // 我的用户信息
                , friend: data.company || [] // 联系人信息
                , group: data.group || [] // 群组信息
                // , company: data.company || {} //公司组织架构
    ...
    var thisChat = LAYIM.prototype.thisChat = function () {
        ...
        return {
            elem: cont
            , data: to
            , textarea: cont.find('.layim-chat-textarea')
        };
    };
    ...
    var sendMessage = function () {
        ...
        if (thatChat.textarea.text().replace(/\s/g, '') !== '') {
            data.content = thatChat.textarea.text();
        ...
        var arrImg = thatChat.textarea.find('img');
        if (arrImg.length > 0) {
            $.each(arrImg, function (i, e) {
                data.content = 'img[' + e.currentSrc + ']';
                ul.append(laytpl(elemChatMain).render(data));
                var param = {
                    mine: data
                    , to: thatChat.data
        ...
        thatChat.textarea.html('').focus();
    };

    // 粘贴截图
    try {
        $('body').on('paste', ".layim-chat-textarea", function (e) {

            var cbd = e.clipboardData || window.clipboardData || e.originalEvent.clipboardData;
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , spread: function (othis) {
            ...
            var local = layui.data('layim')[cache.mine.id] || {};
            othis.nextAll().each(function (index, dom) {
                $(dom)[type === 'true' ? 'removeClass' : 'addClass'](SHOW);
            });
        ...
        , msgbox: function () {
            ...
            msgboxElem.find('span').removeClass(ANIM_MSG).html('0');
        ...
        // 关于
        , about: function () {
            layer.alert('版本： ' + v + '<br>版权所有：<a href="http://www.tengzhicn.com/" target="_blank">www.tengzhicn.com</a>', {
                title: '关于 LayIM'
                , shade: false
            });
        }
        ...
        , chat: function (othis) {
            ...
            if (type === 'friend') {
                ...
                function filterUser(company) {
                    var users = [];
                    $(company).each(function (index, dept) {
                        if (null != dept.groups && dept.groups.length > 0) {
                            users = users.concat(filterUser(dept.groups));
                        }
                        if (null != dept.list && dept.list.length > 0) {
                            users = users.concat($(dept.list).filter(function (iindex, iitem) {
                                return String(iitem.id) === String(id);
                            }).toArray());
                        }
                    });
                    return users;
