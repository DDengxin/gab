package com.tengzhi.business.sc.requisition.materiel.service.impl;
...
import com.tengzhi.business.sc.requisition.materiel.dao.MaterielDao;
import com.tengzhi.business.sc.requisition.materiel.service.MaterielService;
...
public class MaterielServiceImpl implements MaterielService {
    ...
    public BasePage<EckOut> findAll(BaseDto baseDto) throws IOException {
        ...
        String sqlWhere = " where out_act in (select param_id from sys_param where param_name ='" + WarehouseActionCaliber.scly + "' )   ";
        ...
        String sql = "select f_getname('GETDWNAME',out_customer,'',data_corp)out_customer,out_rq ,out_note  ,f_get_param_name('库房档案',out_lib,'"+   SessionUser.getCurrentCorpId()   +"','cn')out_lib,"
                + "out_act ,sum(out_js) out_js ,sum(out_sl) out_sl,sum(out_zl) out_zl,out_flag ,f_getname('GETUSERNAME',data_man,'',data_corp)data_man ,MAX(data_date) data_date,data_corp  "
    ...
    public Map<String, Object> getKcList(BaseDto baseDto) throws IOException {
        ...
        if ("DBZFH".equals(fhStype)) {
            sql = "	select  a.code,f_getparamname('GETBCPCODENAME',a.cpcode_name,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"')cpcode_name,"
            		+ "a.cpcode_size,sum(a.js)js,sum(a.sl)sl, a.in_bpack bpack,a.cpcode02 out_hs,0 out_price,'RMB' out_bz,0 out_tax,a.cpcode_type out_type,cpcodeflname cpcode_fl from v_xnkc_item a    where   " + sqlWhere +
            ...
            sql = "	select  a.code,f_getparamname('GETBCPCODENAME',a.cpcode_name,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"')cpcode_name,"
            		...
            		+ ""
            		+ ",a.js,a.sl,a.pack,a.in_bpack bpack,a.cpcode02 out_hs,0 out_price,'RMB' out_bz,0 out_tax,a.cpcode_type out_type,cpcodeflname cpcode_fl from v_xnkc_item a   " + sqlWhere;
    ...
    public BasePage<Map<String, Object>> getOutList(BaseDto baseDto) throws IOException {
        ...
        String sql = "select out_contract  ,out_code,"
        		...
        		+ "cpcode_size,cpcode_size_en,"
        		+ "f_getparamname('GETCPCODESIZE',cpcode_size,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_name, "
        		+ "f_getparamname('GETCPCODESIZEEN',cpcode_size_en,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_size_en_name,"
        		+ "f_getparamname('GETCPCODEXP',cpcode_xp,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"')  cpcode_xp_name,"
        		+ "out_js,out_sl,out_pack,out_bpack,out_hs, out_price, out_bz, out_tax,"
        		+ "f_getparamname('GETBYCPCODEFL',b.cpcode_fl,'','技术',out_type,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_fl from e_ck_out a ,e_js_cpcode b  where a.out_code=b.cpcode_id  " + sqlWhere;
        if (ObjectUtil.isNotEmpty(map.get("fhStype"))) {
            if ("DBZFH".equals(map.get("fhStype"))) {
                sql = "select out_contract  ,out_code,f_getparamname('GETBCPCODENAME',b.cpcode_name,'','技术',cpcode_type,'"+   SessionUser.getCurrentCorpId()   +"')cpcode_name,cpcode_size,cpcode_size_en,sum(out_js)out_js,sum(out_sl)out_sl,out_bpack,out_hs, out_price, out_bz, out_tax,f_getparamname('GETBYCPCODEFL',b.cpcode_fl,'','技术',out_type,'"+   SessionUser.getCurrentCorpId()   +"') cpcode_fl from e_ck_out a ,e_js_cpcode b  where a.out_code=b.cpcode_id  " + sqlWhere
      ...
      //  return materielDao.QueryPageLists(baseDto, sql);
        return  materielDao.QueryMapPageList(baseDto, sql, true);
    }
    ...
    public Result save(ECKOutVo eCkOutVo) throws Exception {
        ...
        String parameterId = materielDao.getSingleResult("select f_get_in_out_act('GETACT',?1,?2,?3) "
                , WarehouseActionCaliber.scly,eCkOutVo.geteCkOut().getOutType(),securityUser.getCorpId());
        SysParams sysParams = sysParamService.findByParameterId(parameterId, "仓库收发");
        String note = sysGenNoteService.getInOutNote(ECkOut.class,eCkOutVo.geteCkOut().getOutType(), WarehouseActionCaliber.scly,securityUser.getCorpId());
    ...
    public List<EckOut> getDetailByBpack(String bpack, String contract) {
        ...
        return materielDao.QueryListModel(EckOut.class, "   select code out_code,sl out_sl,js out_js ,pack out_pack,in_bpack out_bpack,in_price out_price,xs.ht_tax out_tax,xs.ht_currency out_bz,xs.ht_no out_contract from v_ck_mx mx ,e_xs_contract xs where    in_bpack= :1  and xs.ht_no = :2", bpack, contract);
    }
