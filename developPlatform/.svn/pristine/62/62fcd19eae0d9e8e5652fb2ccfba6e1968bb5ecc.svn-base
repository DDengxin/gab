...
public class TelBookServiceImpl implements TelBookService {
    ...
    public BasePage<Map<String,Object>> findAll(BaseDto baseDto) throws IOException {
        ...
        if ("true".equalsIgnoreCase(String.valueOf(map.get("currentOrgId")))) {
            sqlshere += " and org_id = '" + SessionUser.SessionUser().getCorpId() + "'";
        }

        if (StringUtils.isNotEmpty(map.get("workCorpid"))) {
            sqlshere += " and org_id = '" + map.get("workCorpid") + "'";
        }
...
//                "SELECT dept_id worker_id,parent_id worker_corpid,dept_name corp_exp,' ' worker_phone,' ' worker_email,' ' worker_corp_id,' ' worker_fax_number,' ' worker_fixed_line_telephone,' ' worker_sex FROM sys_dept UNION ALL \n" +
//                "SELECT worker_id,worker_dept,worker_name,worker_phone,worker_email,worker_corpid worker_corp_id,to_char(worker_fax_number,'') worker_fax_number,worker_fixed_line_telephone,worker_sex FROM e_hr_worker) A ";
  //      return telBookDao.QueryPageLists(baseDto, sql);
        String sql ="SELECT a.*  FROM (SELECT dept_id worker_id,parent_id worker_dept,dept_name worker_name,'' worker_phone,'' worker_fax_number,'' worker_fixed_line_telephone,'' worker_sex,'' worker_we_chat,org_id  FROM sys_dept   UNION ALL \n" +
                "SELECT worker_id,worker_dept,worker_name,worker_phone,worker_fax_number,worker_fixed_line_telephone,worker_sex,worker_we_chat,worker_corpid FROM e_hr_worker WHERE worker_flag='启用'  ) a" +sqlshere;
        return telBookDao.QueryToMapPage(baseDto,sql);
    }
    ...
    public List<SelectVo> getDeptWorker(String deptId) {
        ...
        List<Map> mapList = telBookDao.QueryListMap(sql);
        mapList.forEach(Map -> {
