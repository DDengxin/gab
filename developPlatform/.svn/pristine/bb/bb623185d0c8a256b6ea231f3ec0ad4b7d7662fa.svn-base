package com.tengzhi.business.production.subcontract.wwdz.service.impl;
...
@Service
@Transactional
public class WwdzServiceImpl implements WwdzService {
    ...
    public Map<String, Object> getYSList(BaseDto baseDto) throws IOException {
        ...
        if(ObjectUtil.isNotEmpty(map.get("srchDw"))) {
            ...
            String sfkXtype = "";
            String data_corp = sessionUser.getCorpId();
            ...
            String paramIds = "";
            List<Map> list = eCwYsyfDao.QueryListMap(" select param_id from Sys_param where param_type='仓库收发' and param_name='"+ WarehouseActionCaliber.wwrk+"'  ");
            if(list.size() > 0){
            ...
            String actWhere = "";
            if(paramIds.trim().length() != 0)actWhere = " and cw_act in ('"+paramIds+"') ";
            ...
            String sql = " select cw_dw,cw_bz,cw_rq,cw_note,f_getname('GETDWEXP',cw_dw,'','"+sessionUser.getCorpId()+"')dwname ,sum(qc)qc,sum(bqfs)bqfs,sum(bqsfk)bqsfk,(coalesce(sum(qc),0)+coalesce(sum(bqfs),0)-coalesce(sum(bqsfk),0))qmje,sum(bqkp)bqkp from ("
                    + "	 select  cw_dw,cw_bz,current_date cw_rq,cw_act,'上期' cw_note,cw_sje qc,0 bqfs,0 bqsfk ,0 bqkp,'' remarks from e_cw_ysyf  where cw_flag='确认' and  cw_rq<='"+rq1+"'  and cw_dw ='"+ dw+"' and  cw_stype = '"+cwStype+"' and data_corp='"+data_corp+"' "+actWhere+
                    " union all  " +
                    " select  sfk_dw,sfk_bz,current_date,'','上期',sfk_bzje*(-1),0,0,0 ,'' from e_cw_sfk  where sfk_flag='确认' and sfk_rq<='"+rq1+"'  and sfk_dw ='"+ dw+"'  and  sfk_xtype = '"+sfkXtype+"' and data_corp='"+data_corp+"' and sfk_type='WW'  "+
                    " union all " +
                    " select  cw_dw,cw_bz,cw_rq,cw_act,cw_note,0,cw_sje,0,case when cw_fph='N' then 0 else cw_sje end bqkp,'' from e_cw_ysyf  where   cw_flag='确认' and cw_rq>='"+rq1+"' and cw_rq<='"+rq2+"' and cw_dw ='"+ dw+"'  and  cw_stype = '"+cwStype+"' and data_corp='"+data_corp+"'  "+actWhere+
                    " union all  " +
                    " select  sfk_dw,sfk_bz,sfk_rq,'',sfk_note,0,0,sfk_bzje,0 ,sfk_mess from e_cw_sfk  where sfk_flag='确认' and  sfk_rq>='"+rq1+"' and sfk_rq<='"+rq2+"' and sfk_dw ='"+ dw+"'  and  sfk_xtype = '"+sfkXtype+"' and data_corp='"+data_corp+"'  and sfk_type='WW'  "
